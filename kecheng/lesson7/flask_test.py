...
def login():
	...
	if user_action.validate_login(username, password):
		...
		#登陆失败，返回错误信息回页面
		return render_template('userlogin.html', username=username, error="username or password error!!")
...
def writeuser():
	...
	gender = parms.get('gender', '1')
	hobby = parms.getlist('hobby')
