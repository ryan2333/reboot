...
def users():
    ...
    return render_template('users.html', users=models.User.get_users(), msg=request.args.get('msg', ''))            #加载渲染users.html模板
...
def add_user():
    ...
    # _user = models.User()
    # _is_ok, _error = _user.validate_add2(username=username, password=password, passwordrep=passwordrep, age=age, phone=phone, email=email)
    # if _is_ok:
    #     _user.save()

    #检查用户信息
    _is_ok, _error = models.User.validate_add_user(username, password,passwordrep, age, phone, email)
    if _is_ok:
...
def modify_user():
    ...
    _user = models.User.get_user_by_id(uid)
    _error = ''
    _uid = ''
    _username = ''
    _password = ''
...
def change_user_password():
    ...
    _is_ok, _error = models.User.validate_change_user_password(uid, npasswd, session['user']['username'], mpasswd)
    if _is_ok:
        models.User.change_user_password(uid, npasswd)
    return json.dumps({'is_ok': _is_ok, 'error':_error})
