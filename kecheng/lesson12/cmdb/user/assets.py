...
返回None
'''
def validate_create(sn, ip, hostname, os, purchase_date, warranty, vendor, model, idc_id, admin, application, business, cpu, mem, disk):
    _cnt, _rt_list=get_by_id('sn',sn)
    error = {'error':[]}
    rt = True
    if _cnt:
...
返回True/False
'''
def create(sn, ip, hostname, os, purchase_date, warranty, vendor, model, idcs, admin, application, business, cpu, mem, disk):
...
'''
def validate_update(ip, hostname, os, purchase_date, warranty, vendor, model, idc_id, admin, application, business, cpu, mem, disk):
    error = {'error': []}
    rt = True
    iplist = ip.split('.')
    for i in iplist:
...
返回True/False
'''
def update(ip, hostname, os, purchase_date, warranty, vendor, model, idc_id, admin, application, business, cpu, mem, disk, sn):
    _sql = 'update assets set ip=%s, hostname=%s, os=%s, purchase_date=%s, warranty=%s, vendor=%s, model=%s, idc_id=%s, admin=%s, application=%s, business=%s, cpu=%s, mem=%s, disk=%s where sn=%s'
    args = (ip, hostname, os, purchase_date, warranty, vendor, model, idc_id, admin, application, business, cpu, mem, disk, sn)
    execute_commit_sql(_sql, args)
    return True
